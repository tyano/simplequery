...
class LimitExpressionSpec extends FlatSpec with ShouldMatchers with AWSSecurityCredential {
  val simpleDB = getAmazonSimpleDB
  val limitExpression = new Select(simpleDB, "*").from(classOf[TestDomain]).where("name", like("yano%")).orderBy("name", SortOrder.Asc).limit(10)

  "limit() for OrderByExpression" should "generate 'limit 10'" in {
    val expression = new Select(simpleDB, "*").from(classOf[TestDomain]).where("name", like("yano%")).orderBy("name", SortOrder.Asc).limit(10)
  ...
  "limit() for WhereExpression" should "generate 'limit 10' after where expression" in {
    val expression = new Select(simpleDB, "*").from(classOf[TestDomain]).where("name", like("yano%")).limit(10)
  ...
  "limit() for DomainExpression" should "generate 'limit 10' after domain expression" in {
    val expression = new Select(simpleDB, "*").from(classOf[TestDomain]).limit(10)
